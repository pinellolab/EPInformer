...
from .contact import load_hic, load_avg_hic, get_contacts_for_pairs, qc_hic
...
def predict_abc(
    ...
    if hic_file is not None:
        # Auto-detect the Hi-C source type:
        #   juicebox = a single ``.hic`` file (read via hicstraw)
        #   avg      = a directory of per-chromosome ABC average-Hi-C tables
        #              (ENCODE ENCFF134PUN split by scripts/split_avg_hic.py)
        _ht = hic_type
        if _ht == "auto":
            _ht = "avg" if os.path.isdir(hic_file) else "juicebox"
        if _ht == "avg":
            _log(f"Loading ABC average Hi-C from {hic_file} (per-chromosome) ...")
            hic_data = load_avg_hic(hic_file, resolution=hic_resolution)
        else:
            _log(f"Loading Hi-C data from {hic_file} (juicebox .hic) ...")
            hic_data = load_hic(
                hic_file,
