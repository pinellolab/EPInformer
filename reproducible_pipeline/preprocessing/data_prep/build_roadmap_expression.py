...
def main() -> None:
    ...
    parser.add_argument(
        "--gene-bed",
        default=None,
        help="hg38 gene annotation BED (e.g. CollapsedGeneBounds.Ensembl_v65.hg38.pc_linc.bed). "
             "If provided, replaces hg19 coordinates from Roadmap gene_info with "
             "hg38 liftover coordinates. Strongly recommended for pc_linc gene set.",
    )
    parser.add_argument(
        "--log-transform",
    ...
    # ------------------------------------------------------------------
    # 5b. Replace hg19 coords with hg38 from gene BED (if provided)
    # ------------------------------------------------------------------
    if args.gene_bed:
        bed_path = os.path.abspath(args.gene_bed)
        print(f"  Replacing coordinates with hg38 from {bed_path} ...")
        bed = pd.read_csv(
            bed_path, sep="\t", header=None,
            names=["chr", "start", "end", "symbol", "score", "strand", "ensid", "biotype"],
        )
        bed["chrom"] = bed["chr"].str.replace("chr", "", regex=False)
        bed_coords = bed[["ensid", "chrom", "start", "end", "strand"]].drop_duplicates(
            subset=["ensid"], keep="first"
        )
        n_before = len(result)
        result = result.drop(columns=["chrom", "start", "end", "strand"]).merge(
            bed_coords, left_on="ENSID", right_on="ensid", how="inner",
        ).drop(columns=["ensid"])
        print(f"  {len(result)}/{n_before} genes matched hg38 BED coordinates")
