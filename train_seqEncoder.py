...
def train(net, train_ds, fold_i, saved_model_path="./models/",
    ...
    criterion = nn.MSELoss() if loss_type == "mse" else L1KLmixed()
...
def main():
    ...
    parser.add_argument("--test-strand", choices=["auto", "forward", "reverse"],
                        default="auto",
    ...
    parser.add_argument("--loss", type=str, default="mse",
                        choices=["mse", "l1kl"],
                        help="Loss function: mse (default) or l1kl")
    parser.add_argument("--skip-train", action="store_true",
                        help="Eval-only: skip training, load existing best checkpoints "
                             "and just run the test pass (e.g. to re-evaluate a recipe "
                             "under a different --test-strand / --rc-average).")
    ...
    # In --skip-train mode, load checkpoints from --checkpoint-dir (falls back to model_dir);
    # predictions still go to this run's pred_dir so a re-eval never clobbers the source run.
    ckpt_load_dir = args.checkpoint_dir or model_dir

    if args.test_strand == "auto":
        test_strand = "forward" if args.rc_average else "reverse"
    else:
        test_strand = args.test_strand
    ...
    for cell in cell_list:
        ...
        for fi in fold_range:
            ...
            preds_df = test(model, test_ds, model_name=model.name,
                            saved_model_path=ckpt_load_dir, fold_i=fi,
        ...
        for test_set_name in dict.fromkeys(tag for tag, _ in cell_results):
            ...
            # Fold-qualify the filename so one-fold-per-array-task runs do not
            # overwrite each other (subset carries a 'fold' column per row).
            folds_tok = "_".join(str(int(x)) for x in sorted(pd.unique(subset["fold"])))
            out_path = os.path.join(
                pred_dir, f"fold_{folds_tok}_enhancer_predictor_H3K27ac_256bp_{cell}_{test_strand}{suffix}_predictions.csv")
    ...
    # --- Write aggregate summary (flock-append so one-fold-per-array-task runs
