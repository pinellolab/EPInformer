...
class EPInformer_v2(nn.Module):
    def __init__(self, base_size = 4, n_encoder=3, out_dim=128, head = 4, pre_trained_encoder= None, n_enhancer=50, device='cuda', useBN=True, usePromoterSignal=True, useFeat=True, n_extraFeat=0, useLN=True):
        ...
        if pre_trained_encoder is not None:
            ...
            self.name = 'EPInformer_v0.preTrainedConv.{}dim'.format(out_dim)
        else:
            self.seq_encoder = seq_256bp_encoder(base_size=base_size)
            self.name = 'EPInformer_v0.{}dim'.format(out_dim)
        ...
        if self.useFeat:
            if self.usePromoterSignal:
                ...
                feat_n = 8
            self.rna_embed = nn.Linear(feat_n, self.out_dim)
...
#         print(enhancers_padding_mask)
        e_feats = extraFeat #[:, 1:, :]
        e_embed = pe_seq #[:, 1:, :]
        e_embed = self.seq_encoder(e_embed)
        e_embed = self.conv_out(e_embed)
        e_embed = torch.flatten(e_embed.permute(0, 2, 1, 3), start_dim=2)
        if extraFeat is not None:
            e_embed = self.add_pos_conv(torch.concat([e_embed, e_feats], axis=-1).permute(0,2,1)).permute(0,2,1)
        if rna_feat is not None:
            p_embed = self.rna_embed(rna_feat[:, :8]) # remove promoter activity
        pe_embed = torch.cat([p_embed.unsqueeze(1), e_embed], dim=1)
        attn_list = []
        for i in range(self.n_encoder):
            pe_embed, attn = self.attn_encoder[i](pe_embed, enhancers_padding_mask=enhancers_padding_mask, attn_mask=None)
            attn_list.append(attn.unsqueeze(0))
        p_embed = torch.flatten(pe_embed[:,0,:], start_dim=1)
        # if self.useFeat:
        #     p_embed = torch.cat([p_embed, rna_feat], dim=-1)
        p_expr = self.pToExpr(p_embed)
        return p_expr, torch.cat(attn_list)

class EPInformer_old(nn.Module):
    def __init__(self, base_size = 4, n_encoder=3, out_dim=128, head = 4, pre_trained_encoder= None, n_enhancer=50, device='cuda', useBN=False, usePromoterSignal=True, use_rna_feats=True, n_enh_feats=1, useLN=True):
        super(EPInformer_old, self).__init__()
        self.n_enhancer = n_enhancer
        self.out_dim = out_dim
        ...
        self.base_size = base_size
        self.useLN = useLN
        self.use_hk_tok = False
        if pre_trained_encoder is not None:
            self.seq_encoder = pre_trained_encoder
            self.model_name = 'EPInformer_old.preTrainedConv.{}dim'.format(out_dim)
        else:
            self.seq_encoder = seq_256bp_encoder(base_size=base_size)
            self.model_name = 'EPInformer_old.{}dim'.format(out_dim)
        self.n_encoder = n_encoder
        self.device = device
        self.attn_encoder = get_clones(MHAttention_encoderLayer(d_model=out_dim, nhead=head), self.n_encoder)
        attn_mask = (~np.identity(self.n_enhancer+1).astype(bool))
        ...
        self.rna_embed = nn.Linear(8, self.out_dim)
        self.pToExpr = nn.Sequential(
                    nn.Linear(self.out_dim, 128),
                    nn.ReLU(),
                    nn.Linear(128, 128),
        ...
        feat_conv = nn.Sequential(
                nn.Conv1d(in_channels = self.out_dim+1, out_channels=self.out_dim, kernel_size=1),
                nn.ReLU(),
                nn.Conv1d(in_channels = self.out_dim, out_channels=self.out_dim, kernel_size=1),
                nn.ReLU(),
        )
        self.add_feats = get_clones(feat_conv, self.n_enh_feats)
...
#         print(enhancers_padding_mask)
        # e_feats = extraFeat# [:, 1:, :]
        # e_embed = pe_seq# [:, 1:, :]
        e_embed = self.seq_encoder(e_seqs)
        e_embed = self.conv_out(e_embed).permute(0, 2, 1, 3)
        # e_embed = torch.flatten(e_embed.permute(0, 2, 1, 3), start_dim=2)
        # if extraFeat is not None:
        e_embed = torch.flatten(e_embed, start_dim=2) # torch.cat((torch.flatten(e_embed, start_dim=2), e_feats), axis=-1)
        # torch.cat((torch.flatten(e_embed, start_dim=2), e_feats), axis=-1)
        for i in range(self.n_enh_feats):
            # torch.concat([e_embed, e_feats], axis=-1)
            e_embed = torch.concat((e_embed, e_feats[:, :, [i]]), axis=-1)
            e_embed = self.add_feats[i](e_embed.permute(0,2,1)).permute(0,2,1) 
        if rna_feats is not None:
            p_embed = self.rna_embed(rna_feats[:, :8]).unsqueeze(1) # remove promoter activity
        pe_embed = torch.cat((p_embed, e_embed), axis=1)
        attn_list = []
        for i in range(self.n_encoder):
            pe_embed, attn = self.attn_encoder[i](pe_embed, enhancers_padding_mask=padding_mask, attn_mask=self.attn_mask.to(self.device))
            attn_list.append(attn.unsqueeze(0))
        p_embed = pe_embed[:, 0, :]
        # if rna_feats is not None:
