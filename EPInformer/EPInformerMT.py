...
class EPInformerMT(nn.Module):
    def __init__(self, base_size = 4, n_encoder=3, out_dim=64, head = 4, pre_trained_encoder= None, n_enhancer=50, device='cuda', 
                 useBN=True, usePromoterSignal=True, useFeat=True, n_enh_feats=0, useLN=True, n_cell=10):
        super(EPInformerMT, self).__init__()
        self.n_enhancer = n_enhancer
        self.out_dim = out_dim
        self.n_cell = n_cell
        self.useFeat = useFeat
        self.model_name = 'EPInformerMT'
        ...
        self.device = device
        if useLN:
            self.attn_encoder = get_clones(MHAttention_encoderLayer(d_model=256, nhead=8), self.n_encoder)
        else:
            self.attn_encoder = get_clones(MHAttention_encoderLayer_noLN(d_model=256, nhead=8), self.n_encoder)
    ...
    def forward(self, p_seq, e_seqs, cell_tok, e_feats, rna_feat=None, ):
...
#         print(enhancers_padding_mask)
        ...
        p_embed = torch.flatten(p_embed, start_dim=2) + self.cell_embed(cell_tok)
        ...
        for i in range(self.n_encoder):
            pe_embed, attn = self.attn_encoder[i](pe_embed, enhancers_padding_mask=padding_mask)
