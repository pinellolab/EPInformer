...
class ContinuousValueEncoder(nn.Module):
    ...
    def __init__(self, d_model: int = 32, dropout: float = 0.1, max_value: int = 512):
        super().__init__()
...
class EPInformer_abc(nn.Module):
    def __init__(self, base_size = 4, n_encoder=3, out_dim=128, head = 4, pre_trained_encoder= None, n_enhancer=50, device='cuda', useBN=False, usePromoterSignal=True, useFeat=True, n_extraFeat=0, useLN=True):
        ...
        attn_mask[:, 0] = False
        attn_mask[0, :] = False
        attn_mask = torch.BoolTensor(attn_mask.tolist())
    ...
    def forward(self, pe_seq, rna_feats=None, enh_feats=None):
        ...
        for i in range(self.n_encoder):
            pe_flatten_embed, attn = self.attn_encoder[i](pe_flatten_embed, enhancers_padding_mask=enhancers_padding_mask, attn_mask=self.attn_mask.to(pe_flatten_embed.device))
...
class EPInformer_abc_dist_v2(nn.Module):
    ...
    def forward(self, pe_seq, rna_feats=None, enh_feats=None):
        ...
        if self.n_extraFeat > 1:
            ...
            # # print(feats_w.shape)
            # print(feats_w[0])
            # dist_w = dist_w.masked_fill(enhancers_padding_mask[:, 1:], float('-inf'))
            # dist_w = F.softmax(dist_w, dim=-1)
            # print(F.softmax(feats_w))
            # print(feats_w[0])
            feats_w = torch.cat((enh_feats[:, [0], 1], torch.nan_to_num(feats_w, nan=0.0)), dim=1) # add promoter feats
            # print(feats_w.shape)
            # print(feats_w[0])
            # dist_w = torch.cat((enh_feats[:, [0], 0], dist_w), dim=1) # add promoter feats
            # feats_w = dist_w*feats_w
            # print(feats_w.shape) 
            # feats_w = F.softmax(feats_w, dim=-1) # normalize the feature weights
            p_embed = feats_w.unsqueeze(1) @ pe_flatten_embed
            # pd_embed = dist_w.unsqueeze(1) @ pe_flatten_embed
            # p_embed = (pf_embed + pd_embed).flatten(start_dim=1) # ABC-enhanced Attention score
