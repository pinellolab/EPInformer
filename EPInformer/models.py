...
class EPInformer_v2(nn.Module):
    ...
    def forward(self, pe_seq, rna_feats=None, enh_feats=None):
        ...
        # Slot 0 is the promoter — the prediction anchor (p_embed = output[:,0]). Force it
        # to always be attendable, even if its sequence is all-N/empty (a handful of genes
        # near assembly gaps). Otherwise a gene with an empty promoter AND no enhancers has
        # every attention key masked -> softmax over all -inf -> NaN, which poisons training.
        # No-op for genes with a valid promoter (slot 0 already unmasked).
        enhancers_padding_mask[:, 0] = False
