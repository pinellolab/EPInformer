...
def _apply_tss_column(cage_xpresso_df: pd.DataFrame, tss_column: str) -> pd.DataFrame:
    ...
    Falls back to strand-aware TSS (gene ``start`` for + strand, ``end`` for
    - strand) if the requested TSS column is missing (e.g. when using pure
    Roadmap expression without Xpresso features).
    """
    if tss_column not in cage_xpresso_df.columns:
        # Requested TSS column missing — compute strand-aware TSS from gene coords
        if "start" in cage_xpresso_df.columns and "strand" in cage_xpresso_df.columns:
            print(f"  WARNING: TSS column {tss_column!r} not found; "
                  f"computing strand-aware TSS from gene start/end")
            cage_xpresso_df = cage_xpresso_df.copy()
            cage_xpresso_df["TSS"] = np.where(
                cage_xpresso_df["strand"] == "+",
                cage_xpresso_df["start"],
                cage_xpresso_df["end"],
            )
