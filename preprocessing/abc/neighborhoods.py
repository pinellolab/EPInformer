...
def _count_reads_pooled(bam_paths, regions: pd.DataFrame, n_threads: int = 1) -> pd.DataFrame:
    """Count reads in *regions* across one or more replicate BAMs.
    ...
    reps = [count_reads_in_regions(b, regions, n_threads=n_threads).reset_index(drop=True)
            for b in bam_paths]
    if len(reps) == 1:
        return reps[0]
    out = reps[0].copy()
    stack = np.stack([r[["readCount", "RPM", "RPKM"]].values for r in reps], axis=0)  # (R, N, 3)
    out[["readCount", "RPM", "RPKM"]] = stack.mean(axis=0)
    return out


def _count_signal(bam_paths, regions: pd.DataFrame, prefix: str, n_threads: int = 1) -> pd.DataFrame:
    """Count reads and rename columns with *prefix* (e.g. 'DHS' or 'H3K27ac').

    *bam_paths* may be a single BAM path or a list of replicate BAMs
    (per-rep RPM mean-pooled).
    """
    result = _count_reads_pooled(bam_paths, regions, n_threads=n_threads)
    rename_map = {
...
def quantify_neighborhoods(
    ...
    accessibility_bams : list[str], optional
        Multi-rep override for ``accessibility_bam``.  When provided, RPM is
        averaged across these biological-replicate BAMs so the ABC activity
        uses the SAME BAMs as the encoder pre-training activity target.
    h3k27ac_bams : list[str], optional
        Multi-rep override for ``h3k27ac_bam`` (per-rep RPM mean-pooled).
    ...
    # Prefer replicate lists (mean-pooled) so ABC activity matches the encoder's
    # activity target; fall back to the single BAM when no list is given.
    dhs_bams = accessibility_bams if accessibility_bams else accessibility_bam
    h3k_bams = h3k27ac_bams if h3k27ac_bams else h3k27ac_bam
    has_h3k27ac = bool(h3k_bams)
    if logger is not None:
        if accessibility_bams and len(accessibility_bams) > 1:
            logger.info(f"DNase activity: mean-pooling {len(accessibility_bams)} replicate BAMs")
        if h3k27ac_bams and len(h3k27ac_bams) > 1:
            logger.info(f"H3K27ac activity: mean-pooling {len(h3k27ac_bams)} replicate BAMs")
    ...
    candidates = _count_signal(dhs_bams, candidates, "DHS", n_threads=n_threads)

    if has_h3k27ac:
        _log(f"Counting H3K27ac reads in {n_cand} candidates (threads={n_threads}) ...")
        candidates = _count_signal(h3k_bams, candidates, "H3K27ac", n_threads=n_threads)
    ...
    promoter_counts = _count_reads_pooled(dhs_bams, promoter_df, n_threads=n_threads)
    gene_df["DHS.RPM.TSS1Kb"] = promoter_counts["RPM"].values

    if has_h3k27ac:
        promoter_counts_h3k = _count_reads_pooled(h3k_bams, promoter_df, n_threads=n_threads)
